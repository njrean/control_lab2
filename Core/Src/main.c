...
void ADCConfigInit()
{
	//Config ADC Channel PA7
	ADCChannel[0].Config.Channel = ADC_CHANNEL_7;
...
void ADCPollingMethodUpdate()
{
	...
	HAL_ADC_ConfigChannel(&hadc1, &ADCChannel[0].Config); //use pointer &... get address
											// &hadc1 watch at line 42

	//Start Sampling
	HAL_ADC_Start(&hadc1);

	//Wait for Sampling and conversion
	if (HAL_ADC_PollForConversion(&hadc1, 100) == HAL_OK) //Timeout 10 mean read 10ms
	{
		//Read Data
		ADCChannel[0].data = HAL_ADC_GetValue(&hadc1);
		angle = ADCChannel[0].data * 0.083;
	}
